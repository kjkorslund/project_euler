...
public class P8 {
	...
	public static void main(String[] args) {
		//genDigitsArr(digitsStr);
		System.out.println(greatestProduct5Consecutive(digits));
	}
	
	static int greatestProduct5Consecutive(byte[] digits) {
		byte[] lastFive = new byte[5];
		int result = 0;
		
		System.arraycopy(digits, 0, lastFive, 0, 5);
		result = product5(lastFive);
		for(int i=5; i<digits.length; i++) {
			lastFive[i%5] = digits[i];
			result = Math.max(result, product5(lastFive));
		}
		return result;
	}
	
	static int product5(byte[] lastFive) {
		return (int)lastFive[0] * (int)lastFive[1] * (int)lastFive[2] * (int)lastFive[3] * (int)lastFive[4]; 
	}
	
	static void genDigitsArr(String digitsStr) {
		StringBuilder sb = new StringBuilder("static final byte[] digits = new byte[] {");
		for(int i=0; i<digitsStr.length(); i++) {
			...
			sb.append((int)(digitsStr.charAt(i) - '0'));
		}
		sb.append("\n};");
