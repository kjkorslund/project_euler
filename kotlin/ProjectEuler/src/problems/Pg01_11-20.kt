 ...
 * Which starting number (of the Collatz sequence), under one million, produces the longest chain?
 */
object P14: Problem<Pair<Long, Int>> {
    override fun calculate(): Pair<Long, Int> {
        return (1L..1_000_000L)
            .map { it to collatzSequence(it).count() }
            .maxByOrNull { it.second }!!
    }

    private fun collatzSequence(start: Long) : Sequence<Long> = sequence {
        var current = start
        while(current > 1L) {
            yield(current)
            current = when {
                current.isEven() -> current/2
                current.isOdd() -> 3*current + 1
                else -> throw IllegalStateException("$current is neither even nor odd??")
            }
        }
        if (current == 1L) {
            yield(current)
